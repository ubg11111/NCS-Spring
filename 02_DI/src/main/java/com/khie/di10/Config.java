 ...
 *  - 애노테이션의 역할 
 *  	* 컴파일러에게 정보를 알려주는 역할.
 *  	* 컴파일 할 때 설치 시의 작업을 지정하는 역할.
 *  	* 실행할 때에 별도의 처리가 필요한 경우 사용되는 역할.
 *  
 */

@Configuration
@ImportResource("classpath:baseball4.xml") // 자바파일에서 xml을 임포트해오는 방식
public class Config {
	...
	public Player player3() {
		...
		Player player = new Player("강호동", 48, position);
		
		player.setWeight(150);
		player.setHeight(190);
	...
	public Player player4() {
		...
		Player player = new Player("박동환", 38, position);
