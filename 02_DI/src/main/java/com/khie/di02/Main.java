 ...
 *  
 *  - xml을 이용한 스프링 설정을 하다 보면 컨테이너가 생성할 객체를  지정 하기 위해 <bean> 태그를
 *    사용하게 됨.
 *    
 *  - 스프링 컨테이너가 생성해서 보관하는 객체를 스프링 빈(spring bean) 객체라고 부르며,
 *    일반적으로 자바 객체와 동일함.
 *    
 *  - 스프링 컨테이너는 생성한 빈 객체를 <이름(id), 빈 객체(class)> 이렇게 쌍으로 보관을 함.
 ...
 * 					위함이 아닌 스프링을 사용하는 다양한 부가 기능(트랜잭션 처리, 자바 코드
 * 					기반의 스프링설정, 애노테이션을 이용한 빈 설정, 스프링을 이용한 웹 개발
 * 					등등) 때문인데 이러한 부가 기능을 사용하기 위해서는 ApplicationContext
 * 					객체를 주로 사용하게 됨.
 * 
 ...
 * 									을 전달했는데 이는 클래스 패스에 위치한 xml 파일을 설정 파일로
 * 									사용한다는 의미임.
 *  
 *  
 ...
 *  구현할 수 있음. getBean() 메서드의 첫번째 파라미터는 구현하고자 하는 스프링 빈 객체의 고유한 
...
public class Main {
	...
	public static void main(String[] args) {
		...
		// 형변환하는 방법은 첫번째 파라미터만 들어가있는경우....
		// 두번째 파라미터에 ServiceImpl.class를 가져올 수도있다.
